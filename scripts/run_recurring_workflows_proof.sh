...
RUN_ID="${RUN_ID:-recurring_workflows_10seed}"
SEEDS="${SEEDS:-11,22,33,44,55,66,77,88,99,111}"
FAMILY="${FAMILY:-configs/families/recurring_workflows_proof.yaml}"
...
readme = f"""# Proof Artifacts: recurring_workflows, {len(seeds)}-seed run
...
Stream: {params.get("steps")} steps, {params.get("workflows_per_step")} workflows/step, replay queries/workflow {params.get("replay_queries_per_workflow", 0)}
...
- `chart_seed_h2h_full_brain_vs_best_rag.png` -- seed-by-seed dumbbell chart (full_brain vs {best_rag}) showing head-to-head consistency
